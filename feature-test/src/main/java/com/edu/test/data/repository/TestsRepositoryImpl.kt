...
import com.edu.test.data.room.db.mapper.QuestionWithAnswersToCreateQuestionDomainMapper
...
class TestsRepositoryImpl @Inject constructor(
    ...
    override suspend fun uploadTest(groupId: String, testId: String): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                val (test, questions) = testDao.getTestWithQuestions(testId)
                ...
                val response = db.collection("groups").document(groupId).collection("tests").add(
                    Test(
                        ...
                        time = test.duration,
                        title = test.testName,
                        titleKeywords = test.testName.getKeywords()
                    )
                ).await()
                db.runBatch {
                    questions.forEach {
                        db.collection("groups").document(groupId).collection("tests")
                            ...
                            .add(QuestionWithAnswersToCreateQuestionDomainMapper.mapToDomain(it))
                    }
                }.await()
